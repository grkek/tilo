module Tilo
  class Registry
    ...
    property registered_components : Helpers::Synchronized(Hash(String, Tilo::Component)) = Helpers::Synchronized(Hash(String, Tilo::Component)).new
    ...
    def refresh_state(id : String)
      ...
      when "Entry"
        entry = component.widget.as(Gtk::Entry)

        component.state do |state|
          state["horizontalAlignment"] = JSON.parse({"id" => entry.halign.to_i, "name" => entry.halign.to_s}.to_json)
          state["verticalAlignment"] = JSON.parse({"id" => entry.valign.to_i, "name" => entry.valign.to_s}.to_json)
          state["accessibleRole"] = JSON.parse({"id" => entry.accessible_role.to_i, "name" => entry.accessible_role.to_s}.to_json)
          state["canFocus"] = JSON::Any.new(entry.can_focus)
          state["canTarget"] = JSON::Any.new(entry.can_target)
          state["cssClasses"] = JSON.parse(entry.css_classes.to_json)
          state["cssName"] = JSON::Any.new(entry.css_name)
          state["focusOnClick"] = JSON::Any.new(entry.focus_on_click)
          state["focusable"] = JSON::Any.new(entry.focusable)
          state["hasDefault"] = JSON::Any.new(entry.has_default)
          state["hasFocus"] = JSON::Any.new(entry.has_focus)
          state["hasTooltip"] = JSON::Any.new(entry.has_tooltip)
          state["heightRequest"] = JSON::Any.new(entry.height_request.to_i64)
          state["horizontalExpand"] = JSON::Any.new(entry.hexpand)
          state["horizontalExpandSet"] = JSON::Any.new(entry.hexpand_set)
          state["marginBottom"] = JSON::Any.new(entry.margin_bottom.to_i64)
          state["marginEnd"] = JSON::Any.new(entry.margin_end.to_i64)
          state["marginStart"] = JSON::Any.new(entry.margin_start.to_i64)
          state["marginTop"] = JSON::Any.new(entry.margin_top.to_i64)
          state["name"] = JSON::Any.new(entry.name)
          state["opacity"] = JSON::Any.new(entry.opacity)
          state["overflow"] = JSON.parse({"id" => entry.overflow.to_i, "name" => entry.overflow.to_s}.to_json)
        end
      when "Frame"
      when "Image"
        image = component.widget.as(Gtk::Picture)

        begin
          component.state do |state|
            state["resourcePath"] = JSON::Any.new(image.file.to_s)
          end
        rescue exception
    ...
    def register(component : Tilo::Component)
