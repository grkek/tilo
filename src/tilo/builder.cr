module Tilo
  class Builder
    ...
    def build_from_string(string : String)
      ...
      tokenizer = Parser::Tokenizer.new(string, content_directory)
      nodes = tokenizer.parse_nodes

      JavaScript::Engine.instance.sandbox.eval_mutex!("const contentDirectory = \"#{content_directory}\";")
      resolve_paths(content_directory + "/scripts/")

      case nodes.first
      when Application
        application = Gtk::Application.new(application_id: nodes.first.as(Generic).attributes["applicationId"]?.to_s || ["com", "tilo", UUID.random.hexstring].join("."))
        ...
        application.activate_signal.connect do
          ...
          child.attributes["id"] = JSON::Any.new("MainWindow")
    ...
    def build_from_file(file_path : String)
    ...
    def build
      File.open(content_directory + "/pages/index.html") do |fp|
        string = fp.gets_to_end
    ...
    private def transpile_component(child, widget : Gtk::Widget)
      ...
      when Script
        if path = child.attributes["src"]?
          file = File.open(content_directory + "/scripts/" + path.to_s).gets_to_end
          @scripts.push(file)
        end
    ...
    private def process_stylesheet(child)
      ...
      if path = child.attributes["src"]?
        css_provider.load_from_path(content_directory + "/styles/" + path.to_s)
      end
    ...
    private def resolve_paths(path : String)
      ...
      children.each do |child|
        ...
        if File.directory? absolute_path
          resolve_paths(absolute_path)
        end

        if File.file? absolute_path
          JavaScript::Engine.instance.paths.push(absolute_path)
        end
      end
    end
  end
