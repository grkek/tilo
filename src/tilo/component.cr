  ...
  class Component
    ...
    def initialize(@id : String, class_name : String, @kind : String, @widget : Gtk::Widget)
      ...
      when "Box"
        ...
        append = ->(argument : JSON::Any) {
          if component = Registry.instance.registered_components[argument.as_s]
            self.state do |state|
              state["children"] = JSON.parse(state["children"].as_a.push(argument).to_json)
            end

            box.append(component.widget)
        ...
        register_callback("append", 1, append)
        register_callback("destroyChildren", 0, destroy_children)
      ...
      when "Entry"
        ...
        set_text = ->(argument : JSON::Any) {
          ...
          entry.text = argument.as_s
          argument
        }

        get_text = ->{
          text = JSON::Any.new(entry.text)
        ...
        register_callback("setText", 1, set_text)
        register_callback("getText", 0, get_text)
      ...
      when "Image"
        ...
        set_file_path = ->(argument : JSON::Any) {
          self.state do |state|
            state["resourcePath"] = argument
          end

          if argument.to_s.starts_with?("http")
            image.file = Gio::File.new_for_uri(argument.to_s)
          else
            image.file = Gio::File.new_for_path(argument.to_s)
          end

          argument
        }

        register_callback("setResourcePath", 1, set_file_path)
      ...
      when "ListBox"
        ...
        register_callback("removeAll", 0, remove_all)
      ...
      source_code.push(["globalThis[", "\"#{id}\"", "]", ".", "isMounted", " ", "=", " ", "true", ";"].join)
      ...
      self.state do |state|
        state["kind"] = JSON::Any.new(kind.upcase)
      end
    end
    ...
    private def unmount(&)
      ...
      # Direct calls to the evaluation to make it a bit faster.
      source_code.push(["globalThis[", "\"#{id}\"", "]", ".", "isMounted", " ", "=", " ", "false", ";"].join)
      JavaScript::Engine.instance.sandbox.eval_mutex! source_code.join
    ...
    private def update_component(symbol : Symbol, value : String)
      unmount do
        ...
        case symbol
        when :id
          source_code.push(["globalThis[", "\"#{value}\"", "]", " ", "=", " ", "#{id}", ";"].join)
          source_code.push(["globalThis[", "\"#{id}\"", "]", " ", "=", " ", "undefined"].join)

          widget.name = value
          @id = value
        when :class_name
          source_code.push(["globalThis[", "\"#{id}\"", "]", ".", "className", " ", "=", " ", "\"", "#{value}", "\"", ";"].join)
    ...
    private def update_component(symbol : Symbol, value : Helpers::Synchronized(Hash(String, JSON::Any)))
      unmount do
        ...
        case symbol
        when :properties
          source_code.push(["globalThis[", "\"#{id}\"", "]", ".", "properties", " ", "=", " ", value.to_json.gsub("\"", ""), ";"].join)

          @properties = value
        when :state
          source_code.push(["globalThis[", "\"#{id}\"", "]", ".", "state", " ", "=", " ", value.to_json, ";"].join)
